...
int CreateCompositeImage( int xmin, int ymin, int w2, int h2, vector<Picture> &vp,
 uint32 *bw, char col, uint32 *color, uint8 *map, int mapw, vector<TForm> &tfs)
{
int npixels2 = w2*h2;
int nvpix = 0;
...
p.fname = argv[1];
p.tr = TForm();  // make this explicit; already true from initialization
below.push_back(p);
...
for(int j=0; j<below.size(); j++) {
	...
	if( oxmax > oxmin && oymax > oymin ) {
	    ...
	    MultiplyTrans(above[k].tr, below[j].tr, maps.transforms[0]);
	    InvertTrans(above[k].Inverse, above[k].tr);  // Recompute j's inverse
            }
	}
    }
...
for(int q=0; q<maps.centers.size(); q++) {
    TForm temp;
    MultiplyTrans(temp, below[0].tr, maps.transforms[q]); // maps A to global
    InvertTrans(maps.transforms[q], temp);
    }
