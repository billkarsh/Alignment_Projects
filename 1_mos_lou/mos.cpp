...
for(int i=0; i<N*N; i++) {
    ...
    if(dp) printf("sx,sy = %d, %d\n", sx, sy);
    if (!(*LegalRegion)(sx, sy, arg)) {
        rslt[i] = 0.0;
        if(dp) printf("Illegal region\n");
	continue;
	}
    ...
    if(dp)printf(" %d %f %f %f %f, %f, %f %f %f, %f\n", n,
     im1sum, im1sum2, im2sum, im2sum2, num, d1, d2, d, rslt[i]);
    rslt[i] = r;
...
double mi = LookFFT(rslt, N, bigx-1, bigy);
double pl = LookFFT(rslt, N, bigx+1, bigy);
double deltax, deltay; // changes due to interpolation
if (mi == 0.0 || pl == 0.0) {
    printf("WARNING! At edge of legal region.  No X interpolation.\n");
    nwarn_edge_interp++;
    deltax = 0.0;
    }
else {
    mi -= v;  // relative value at -1
    pl -= v;  // same for +1
    double a = (mi+pl)/2;
...
mi = LookFFT(rslt, N, bigx, bigy-1);
pl = LookFFT(rslt, N, bigx, bigy+1);
if (mi == 0.0 || pl == 0.0) {
    printf("WARNING! At edge of legal region.  No Y interpolation.\n");
    nwarn_edge_interp++;
    deltay = 0.0;
    }
else {
...
for(int i=0; i<images.size(); i++) {
    ...
    for(int patch=1; patch < images[i].tf.size(); patch++) {
        ...
        // dp = false;
        double co = FindNormCorrelation(pts1, vals1, pts2, vals2, dx, dy, 0, 0, 4000, stdout,
         lr, NULL, lc, NULL, ftc);
