...
//-----------------------------------------------------
// Find the output file name, if specified.  If not, use "results"
	char	*OutFileName = "results";
	int		i;
	
	for( i = 0; i < argc; ++i ) {
		if( !strncmp( argv[i], "-o=", 3 ) ) {
			OutFileName = argv[i] + 3;
			break;
		}
	}
//-----------------------------------------------------
...
// Read the right hand side from stdin
...
//-----------------------------------------------------
	...
	double	*d  = (double*)((DNformat*)B.Store)->nzval;
